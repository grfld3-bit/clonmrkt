...
if (token) {
  ...
  bot.on('message', async (msg) => {
    ...
    if (text === '/start') {
      bot?.sendMessage(chatId, "Halo! Kirimkan link marketplace 'mrkt' untuk menambahkan item ke marketplace.");
      return;
    }

    if (text?.includes('mrkt.com')) {
      bot?.sendMessage(chatId, "Link diterima! Sedang memproses item...");
      
      try {
        // Use recommended model and correct API call
        const response = await ai.models.generateContent({
          model: "gemini-3-flash-preview",
          contents: `Extract product details from this link: ${text}. Return JSON: name, price (number), imageUrl, number.`,
          config: {
